...
func outputStartupDirective(ctx RoleContext) {
	...
	case RoleDeacon:
		...
		fmt.Println("---")
		fmt.Println()
		fmt.Println("**STARTUP PROTOCOL** (CRON-SPAWN MODE, gt-097): You are the Deacon.")
		fmt.Println("This session does ONE patrol cycle and then exits. The daemon will spawn")
		fmt.Println("a fresh Deacon at its next heartbeat tick (~3 min) — do NOT loop in-process.")
		fmt.Println()
		fmt.Println("1. Run `" + cli.Name() + " prime` (loads full context, mail, and pending work)")
